# LinkedIn profile source

This file is the local source for the LinkedIn profile. Review and approve it before any LinkedIn update.

## Positioning

**Primary narrative:** Product & Technology Executive. I build products, lead product strategy and run product and engineering organisations.

**Primary roles for recruiter search:** CTO, VP Engineering, Head of Engineering, VP Product and Head of Product.

**Secondary offers:** Fractional CTO, technology strategy and advisory work. Keep these in the About section and Services. Do not add them to the headline.

## Headline

Product & Technology Executive | CTO / VP Engineering | Product Strategy & Engineering Leadership

## Banner

Use a neutral personal banner. Do not use a company logo.

```text
Product • Engineering • Technology
Nairobi / Remote
```

## About

I build products and the organisations that bring them to market.

I am the Co-founder and CEO of Sellogram, a work agent for people who sell on Instagram. I lead product strategy and delivery. Sellogram handles customer conversations, product and stock questions, orders, payment confirmations and deliveries.

Before Sellogram, I was CTO at Peach Tech where I led Product, Engineering and IT.

I led the functional, user and ethnographic research, and product design, for the marketplace operating system. Vehicle intake, assessment and yard-management tools were its first delivered components.

At Bridge Technologies, I led product and engineering for payments and cross-border remittances. This included an MTN Group and MFS Africa integration with potential reach to 70M subscribers in Nigeria, and a KCB integration for outbound remittances to Nigeria.

I have 15+ years across fintech, marketplaces, digital health and media. I work with founders and executive teams on product strategy, new product delivery, organisation design and software platforms.

I am open to CTO, VP or Head of Engineering, and product leadership conversations.

## Experience

### Sellogram | Co-founder & CEO

**Jul 2025 – Present | Nairobi, Kenya**

- Lead the team and product strategy for a work agent that supports sellers on Instagram
- Build customer-conversation workflows for product and stock questions, orders, payments and deliveries
- Use posts and reels to extract product information and build product context for social sales teams

### Peach Tech Ltd | Chief Technology Officer

**Dec 2023 – Mar 2025 | Nairobi, Kenya**

- Led Product, Engineering and IT within the Technology Group as an executive-team member
- Defined technology and product strategy for a managed used-car marketplace
- Led delivery of a new web app, vehicle intake, assessment and yard-management tools, and led research and product design for a broader marketplace operating system

### Bridge Technologies Limited | CTO & VP of Engineering

**Jul 2022 – Jan 2023 | Nairobi, Kenya**

- Set product and technology strategy for API-driven payments, remittances, wallets and currency-swap services
- Delivered an MTN Group and MFS Africa cross-border remittance integration with potential reach to 70M subscribers in Nigeria
- Recruited team and reached 80% completion of two consumer apps, a merchant app and a payments and remittance API suite in 3 months
- Delivered a KCB integration that enabled outbound remittances to Nigeria

### Living Goods | Director, Software Engineering

**Oct 2019 – Apr 2021 | Nairobi, Kenya**

- Executed technology strategy for direct operations and technical-assistance programmes in digital health
- Grew a geographically distributed delivery organisation across engineering, data science, QA, design, product and project management
- Defined the software delivery lifecycle with agile methods, continuous delivery and continuous improvement

### Kwara | Chief Technology Officer

**Oct 2018 – May 2019 | Nairobi, Kenya**

- Led a team of 5 developers and designers for a core-banking and operations platform
- Delivered architecture, technical objectives and a product roadmap while supporting marketing, sales and people decisions

### Radio Africa Limited | Group Head of Development

**Jun 2016 – Oct 2018 | Nairobi, Kenya**

- Led a shared-services team of 12 developers, designers and web administrators
- Directed group-wide digital product delivery and advised the Group CEO and executive leaders on digital product strategy and the transition to a digital-first media business
- Built music-streaming and mobile-money billing systems

### Consulting | Consultant

**Sep 2013 – May 2016 | Djibouti, Ghana and Kenya**

- Built data-gathering and analysis tools for NGOs and development organisations
- Designed and implemented an e-commerce platform that supported trade finance under Islamic-banking guidelines

### Genkey Africa Ltd | Lead Software Developer

**Sep 2010 – Sep 2012 | Accra, Ghana**

- Led product-development work for biometric voter-registration systems
- Supported Ghana Electoral Commission delivery and participated in the Kenya IEBC biometric-registration bid team

### CoreNett Ltd | Head of Software Development

**Mar 2007 – Sep 2010 | Accra, Ghana**

- Owned architecture and product development for card payments and vouchers, cardless ATM services and a mobile-wallet product

### Rancard Solutions Ltd | Product Manager, Mobile Platforms

**Jan 2006 – Mar 2007 | Accra, Ghana**

- Managed telco-facing products and built an SMS-voting system for West African Idols

## Education

**Ashesi University College**

B.Sc. Computer Science, Cum Laude | 2005

## Skills

Add these in this order. Pin the first three.

1. Product Strategy
2. Engineering Leadership
3. Product Management
4. Organisation Design
5. Technology Strategy
6. Product Roadmapping
7. LLMs
8. Retrieval-Augmented Generation (RAG)
9. Context Engineering
10. DSPy
11. Enterprise Integration
12. Marketplace Platforms
13. Payments and Remittances
14. Engineering Management
15. Ruby on Rails
16. Kotlin
17. Python
18. TypeScript
19. RabbitMQ

## Featured

Add these only after each item is public and final.

1. Sellogram public product page or short product demo.
2. Product & Technology Executive CV PDF.
3. One-page case study: MTN cross-border remittance or Peach marketplace operating system.

## Job preferences

Use **Recruiters only** visibility.

- Job titles: CTO; VP Engineering; Head of Engineering; VP Product; Head of Product
- Location types: Remote, hybrid and on-site
- Locations: Nairobi; remote roles with Africa and Europe time-zone overlap; other remote roles that meet relocation or work-authorisation requirements
- Employment types: Full-time and contract

Recruiter-only visibility reduces exposure. It does not guarantee privacy from the current company.

## Services

Add this only if fractional or advisory work is active.

- Fractional CTO
- Technology strategy
- Product strategy
- Engineering organisation design
- Technical architecture

## Six-week visibility routine

Each week:

- Add 10 relevant founders, recruiters or product leaders.
- Send 3 tailored connection notes.
- Leave 3 substantive comments on relevant posts.
- Publish 1 short post every 2 weeks.

First post themes:

1. What building Sellogram has taught you about work agents for social commerce.
2. What product and engineering leaders need to get right when building a new operating system.
3. Lessons from cross-border remittances and multi-partner product delivery.

## Review checklist

- Confirm that the public Sellogram website or demo is ready before adding it to Featured.
- Replace any achievement only if you can support it with a source.
- Keep this file and the three base CVs aligned after every career update.
